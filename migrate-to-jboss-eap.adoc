...
we are going to build locally and just upload the artifact (e.g. the `.ear` file). The binary deployment will speed up the build
process significantly.

First, build the project once more using the `openshift` Maven profile, which will create a suitable binary for use with
OpenShift (this is not a container image yet, but just the `.ear` file). We will do this with the `oc` command line.
...
And finally, start the build process that will take the `.ear` file and combine it with JBoss EAP and produce a Linux container
...
You must then get logged in with the testing bankdemo user `200`.
